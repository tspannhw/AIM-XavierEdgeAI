...
try:
    response = client.chat_postMessage(
        channel="C06NE1FU6SE",
        text=(f"NVIDIA Xavier Upload {hostname} {currenttimeofsave} {caption}")
    )
...
try:
    response = client.files_upload_v2(
        channel="C06NE1FU6SE",
        file=strfilename,
        title="NVIDIA Xavier Upload " + str(caption),
        initial_comment="Live Camera image from NVIDIA Xavier of " + str(caption),
    )
except SlackApiError as e:
    assert e.response["error"]
# Slack
